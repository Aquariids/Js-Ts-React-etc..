// promise - Это обещание. Мы говорим  если, что выполнится, то мы обещаем что что то выполним
// promise  - позволяет нам удобно работать с асинхронными операциями.
// с помощью промисов мы работаем с  AJAX

let a = 7; // создаем переменную

console.log(a); // выводим ее
...
console.log(a); // для этого всего у нас есть then. он принимает в себя resolve. и внутри него мы можем работать с данными и просто по цепочке создавать
...
}); // это последовательность действий, при удачной работе мы обещаем, что выплним опр цепочку действий
