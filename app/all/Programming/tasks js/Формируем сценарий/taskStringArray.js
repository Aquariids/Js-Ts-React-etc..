const roles = [`Городничий`,`Аммос Федорович`,`Артемий Филиппович`,`Лука Лукич`,`Христиан Иванович`]; // наши роли в массиве.

// переменная с текстом, который ма поместим в массив и каждую строчку разобьем на элемент
...
const phrases = text.split('\n'); // создаем переменную в которую помещаем наш текст и превращаем его в массив через разделитель /n - это перенос строки
// получим 37 элементов, каждая новая строка после переноса строки.
let result = ''; // сюда будем помещать весь текст который получится в итоге

// далее начинаем строить  наш алгоритм по работе с каждой ролью, мы будем перебирать роли и в зависимости от совпадений будем выводить опр результат

roles.forEach(role => {  // перебираем роли
    let temp = `\n${role}: \n`; // здесь будет наша роль от слова temporary - временный

    phrases.forEach((line,j) => { // далее перебираем наш  массив с текстом
        // у нас два аргумента line и i в пером будут наши строки, во тором нумерация строк
        // далее пишем условие в котором проверяем есть ли у текста наша роль
        if (line.indexOf(role) >= 0 && line.indexOf(role) < role.length) { // условие если в строке есть роль, мы получим позиуию первой буквы 0, если это больше или равно 0, все ок, далее, что бы исклюxить дальнейшие упоминания роли, мы ставим условие, что если индекс найденый нами меньше размера самой роли то все ок. так как есть другие упоминание в тексте ролей, там индекс будет уже далеко не 0.
            // далее формируем строку котую будем отправлять в результат
            const content = line.slice(role.length + 1); // обрезаем нашу роль в тексте
            temp += `${j + 1}) ${content}\n`; // к роли присваиваем весь текст, уже с обрезанной ролью в нем, добавляем индекс + 1 так как у нас все с 0, а нам нужно с единицы начинать

            
        }
         // здесь можно было бы использовать метод line.includes (role) - он бы возвращал нам true если роль есть и false если ее не было бы.  наша роль в тексте может повторяться несколько раз,
        // поэтому он нам не подходит мы будем использовать indexOf ( он будет нам возвращать позицию(индекс) с которой началось совпадение) в нашем случае мы будем получать индекс первой буквы роли, это 0, если там не будет роли мы получим -1
        
        
    });
    result += temp; // добавляем в результат все
});
