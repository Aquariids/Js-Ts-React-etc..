...
console.log(j) // 1
```
Есть такое понятие как ассоциативность - она определяет порядк выполнения операторов с одинаковым приоритетом.
...
## <a name ='incr'> Инкремент и дикремент </a> ##
...
Попытка использовать его на значении 5++ -  приведёт к ошибке.
      
## <a name ='logOperators'> Логические Операторы </a> ##                     
Оператор И '&&' - если хотя бы одно значение будет false, то мы всегда будем получать false.
```javaScript
const isCheck = true,
    isClose = true,
    isFalse = false;
