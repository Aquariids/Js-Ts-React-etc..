...
console.log(-4 / 0); // -Infinity
```
NaN(Not a Number) что значит 'Не число'<br>
...
null - это простой тип данных, но при этом от null унаследованы все объекты. Если проверять тип null то выдаст 'object'. Это все мудреная техническая часть, сейчас не об этом.
```javascript
'use strict';
...
```
#### 4.Регулярные выражения и ошибки ####
...
```  
#### 2.Numbers #### 
К числу приводит унарный + - довольно частый способ
```javaScript
...
```
Так же и арифметические операторы (-, *, /, %). Кроме + с двумя операндами.
```javaScript
    console.log('3' - 2) // 1
    // js привел строку '3' в числовой тип и отнял 2. Получаем цифру 1
```
#### 3.Boolean #### 
Важно запомнить значения и типы которые приводят к логическому типу. Их немного:                                     
Все, что превращается в false!!
```javaScript
const itFalse [0,'',null,undefiend,NaN]; // все это приведет в условии к false
