...
[Объект event]()<br>
[Удаление события и ссылка на функцию]()<br>
[Всплытие событий]()<br>
...
```                                                       
## Ссылка на функцию и removeEventListener() 
...
const deleteElement1 = (event) => { // Создаем нашу функцию в переменной.
    event.target.remove(); // удаляем элемент.
};

btn.addEventListener('click', deleteElement1); //  Передаем в обработчик ссылку на нее вторым аргументом.
btn.removeEventListener('click',deleteElement1); // А тут удаляем обработчик( мы ссылаемся на одну функцию)
