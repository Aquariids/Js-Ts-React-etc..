...
console.log(sum()) // 6
```
Функция sum имела доступ ко всем переменным (a,b,c) и если будет еще функция или условие, то они тоже будет иметь к ним доступ.
Они доступны из любого места программы.
По возможности минимизируем использовние глобальных переменных.
### Локальная облатсь ###
