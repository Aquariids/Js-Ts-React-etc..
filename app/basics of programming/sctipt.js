...
В разных языках разные типы, но основных 3
1 - Число:
int - целое число
float - число с точкой 
Number - например в js, тут будут любые числа, js динамический язык о динам типизации я писал в basic js
примеры в файле types data (на питоне)

2 -  Строка
char - в пайтоне том же
string - в js
записываются в кавычках "" или ''

У каждого символа есть свой номер,и индекс, все идет от 0
'привет'
п - 0
р - 1
и - 2
и тд
получаем их черещщ квадратные скобки


print('abs'[0]) например в пайтоне, тут мы получим в консоле букву a

print('abcdefghij'[0:3]) так мы можем сделать срез, мы получим abc
в js это не работает
*/
...
*                                               ЯЗЫК С
*/
// далее познакомимся с языком C
// а сейчас глянем на ИСХОДНЫЙ КОД - это то что мы пишем в среде
// потом этот код компилируется компилятором и мы получаем в объектный код и все это единицы и нули)

// source code -> compiler -> object code

#include <stdio.h>  // эта строка подключает библиотеку функций #include
 // это наш исходный код на C

int main(void){ // main - это главная функция которая запускает программу
    printf('hello, world\n')   // это метод, функция printf которая принимает в себя входные данные 'hello,world\n'
}
\n - это новая строка, гворим компилятору, что начинаем с новой строки
//clang hello.c - это команда для компиляции кода
// ./a.out - команда для запуска скомпилированного кода
// ./ и имя файла, в моем случае сейчас это hello работаю в ide cs50
// ./hello . - это текущий каталог .. это идти назад на директорию выше
// ./hello - это мы запускаем файл hello  в текущей директории