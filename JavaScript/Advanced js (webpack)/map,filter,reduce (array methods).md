...
### reduce
reduce используется для вычесления какого-нибудь значения на основе всего массива. reduce принимает в себя колбек функцию.
...
Первый - это все сумма(accumulator), второй(currentValue) - это каждый элемент, третий - Это индекс текущего обрабатываемого элемента (index)
```javaScript
